...
def _tokenize_fn(
    text: str, tokenizer: transformers.PreTrainedTokenizer, max_seq_length: int
) -> Dict:
...
def encode_with_prompt_completion_format(
    example, tokenizer, max_seq_length, context_markups=None
):
    """
    ...
    examples_tokenized = _tokenize_fn(
        source_text + target_text, tokenizer, max_seq_length
    )
