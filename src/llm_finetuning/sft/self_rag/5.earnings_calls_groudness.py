...
def process_input(example, multi_retrieval=False):
    if multi_retrieval is False:
        ...
        if (
            "sent_idx" not in example
            or example["sent_idx"] == 0
            or len(example["preceding_sentences"]) == 0
        ):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        help="List of previously processed result files",
    )
    parser.add_argument(
        "--output_file_name", type=str, required=True, help="Output file name"
    )
    parser.add_argument(
        "--multi_retrieval", action="store_true", help="Enable multi-retrieval mode"
    )
    parser.add_argument(
        "--api_key", type=str, help="Path to file containing Groq API key"
    )
    parser.add_argument(
        "--model_name", type=str, default="groq-model", help="Model name for Groq API"
    )
    parser.add_argument(
    ...
    if not args.input_files:
        raise ValueError(
            "No input files provided. Use the --input_files argument to specify input files."
        )

    # Load Groq API key (if necessary)
    if args.api_key:
    ...
    print("previously collected data: {0}".format(len(previous_ids)))
    ...
    task_types = Counter(
        [item["dataset_name"] for item in examples if "dataset_name" in item]
    )
    ...
    for idx, example in tqdm(enumerate(examples)):
        if "output" not in example and "answers" in example:
            example["output"] = (
                example["answers"][0]
                if type(example["answers"]) is list
        ...
        if "instruction" not in example and "question" in example:
            ...
            if data_type in KNOWLEDGE_INSTRUCTIONS:
                example["instruction"] = (
                    KNOWLEDGE_INSTRUCTIONS[data_type] + example["question"]
                )
            else:
        ...
        input_prompt = process_input(example, multi_retrieval=args.multi_retrieval)
        try:
            results = completions_with_backoff(
                model=args.model_name, input_prompt=input_prompt, max_tokens=200
            )
