import argparse
import torch
from peft import AutoPeftModelForCausalLM, PrefixEncoder, PrefixTuningConfig
...
def main(args):
    ...
    trainer = SFTTrainer(
        model=model,
        args=training_args,
        train_dataset=dataset,  # Use dataset from Dataloader
        peft_config=prefix_encoder,
        tokenizer=tokenizer,
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Training and fine-tuning a model using Prefix Tuning")
    ...
    parser.add_argument(
        ...
        choices=["PREFIX_TUNING", "OTHER_TYPE"],
        help="Type of Parameter Efficient Fine Tuning (PEFT).",
    )
    parser.add_argument(
        "--task_type",
        type=str,
        default="CAUSAL_LM",
        choices=["CAUSAL_LM", "SEQ2SEQ_LM", "TOKEN_CLASSIFICATION", "SEQ_CLASSIFICATION"],
        help="Task type for the model.",
    )

    # Prefix tuning specific parameters
    parser.add_argument("--num_virtual_tokens", type=int, default=20, help="Number of virtual tokens to use.")
    parser.add_argument("--token_dim", type=int, default=768, help="Dimension of the token embeddings.")
    parser.add_argument(
        "--num_transformer_submodules",
        type=int,
        default=1,
        help="Number of transformer submodules to apply prefix tuning to.",
    )
    parser.add_argument("--num_attention_heads", type=int, default=12, help="Number of attention heads in the model.")
    parser.add_argument("--num_layers", type=int, default=12, help="Number of layers in the model.")
    parser.add_argument("--encoder_hidden_size", type=int, default=768, help="Hidden size of the encoder.")
