"""MedQA GRPO fine-tuning training script."""
...
from llm_finetuning.medical_question_answering.medqa.data_processing import MedQALoader
...
def main() -> None:
    """Load config, initialise model, and run GRPO training on MedQA."""
    ...
    loader_config = DatasetConfig(
        dataset_id=config.get("dataset_id", MedQALoader.CONFIG.dataset_id),
        subset=config.get("dataset_subset", MedQALoader.CONFIG.subset),
    )
    dataset = MedQALoader(config=loader_config).load(config["dataset_split"])
