"""BioASQ GRPO fine-tuning training script."""
...
from llm_finetuning.medical_question_answering.bioasq.data_processing import (
    BioASQLoader,
)
...
def main() -> None:
    """Load config, initialise model, and run GRPO training on BioASQ."""
    ...
    loader_config = DatasetConfig(
        dataset_id=config.get("dataset_id", BioASQLoader.CONFIG.dataset_id),
        subset=config.get("dataset_subset", BioASQLoader.CONFIG.subset),
    )
    dataset = BioASQLoader(config=loader_config).load(config["dataset_split"])
