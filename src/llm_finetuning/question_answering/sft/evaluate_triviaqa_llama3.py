"""Evaluation script for TriviaQA model."""
...
# Configuration
MODEL_PATH = "./unsloth_sft_triviaqa"  # Path to trained model
DATASET_NAME = "your-hf-username/triviaqa-processed"  # Your dataset
TEST_SPLIT = "test"  # Use "test" or "validation"
MAX_SEQ_LENGTH = 4096  # Match training context length
OUTPUT_DIR = "./evaluation_results_triviaqa"
...
def evaluate_qa(model, tokenizer, dataset, max_new_tokens=100) -> dict:  # type: ignore[no-untyped-def]
    """Evaluate QA performance on TriviaQA dataset."""
    # Load metrics
        ...
        results["samples"].append(
            {
                ...
                "aliases": test_dataset[i]["aliases"],
            }
        )
...
if __name__ == "__main__":
    ...
    with open(SAMPLE_PREDS_PATH, "w") as f:
        f.write(f"TriviaQA Evaluation Results ({len(test_dataset)} samples)\n")
        f.write("=" * 80 + "\n\n")
        ...
        for i, sample in enumerate(eval_results["samples"]):
            ...
            f.write(f"Accepted Aliases: {sample['aliases']}\n")
            f.write("\n" + "=" * 80 + "\n\n")
