...
TEST_SIZE = 0.1
VAL_SIZE = 0.1
HF_REPO_ID = "your-hf-username/hotpotqa-processed"
...
def format_context(context):
    ...
    for i, title in enumerate(context["title"]):
        ...
        context_text += " ".join(context["sentences"][i]) + "\n\n"
...
def preprocess_hotpot(example):
    ...
    for title, sent_id in zip(
        ...
        supporting_facts.append(f"({title}, sentence {sent_id})")
...
if __name__ == "__main__":
    ...
    print(
        f"Dataset sizes: Train={len(hotpot_ds['train'])}, "
        f"Val={len(hotpot_ds['validation'])}, Test={len(hotpot_ds['test'])}"
    )
