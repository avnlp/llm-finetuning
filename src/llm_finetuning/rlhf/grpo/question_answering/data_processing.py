...
def preprocess_hotpot(examples):
    inputs = [
        f"Question: {q}\nContext: {format_hotpot_context(ex)}\nAnswer:" for q, ex in zip(examples["question"], examples)
    ]
    groups = [
        {"answer": ans, "supporting_facts": sf} for ans, sf in zip(examples["answer"], examples["supporting_facts"])
    ]
...
def preprocess_musique(examples):
    ...
    groups = [
        ...
        for ans, aliases, decomp in zip(
            examples["answer"], examples["answer_aliases"], examples["question_decomposition"]
        )
    ]
    return {"query": inputs, "answer": examples["answer"], "group": groups}
