"""PPO training script for preference alignment on WebGPT comparisons."""
...
from llm_finetuning.preference_alignment.ppo.webgpt.data_processing import (
    WebGPTPPOLoader,
)
...
def main() -> None:
    """Run PPO training on WebGPT with pointwise reward model."""
    ...
    loader_config = DatasetConfig(
        dataset_id=config.get("dataset_id", WebGPTPPOLoader.CONFIG.dataset_id),
        subset=config.get("dataset_subset", WebGPTPPOLoader.CONFIG.subset),
    )
    loader = WebGPTPPOLoader(config=loader_config)
    dataset = loader.load(config.get("dataset_split", "train"))
